...
<template>
  ...
  <USlideover v-else>
    <div class="w-full lg:flex flex-col">
      <div class="h-16 px-4 flex items-center gap-2 w-full shrink-0">
        <UIcon name="i-heroicons-chevron-left" @click="slideOver.close()" class="lg:hidden cursor-pointer" />
        <UAvatar src="https://avatars.githubusercontent.com/u/739984?v=4" alt="Avatar" />
        <label>John Doe</label>
      </div>

      <div
        class="h-[calc(100vh-128px)] grid grid-cols-12 gap-4 px-4 py-2 border-y border-gray-200 dark:border-gray-800 overflow-auto dark:text-gray-200">
        <div v-for="i in 30" :key="i" :class="i % 2 == 0 ? 'col-span-8' : 'col-start-4 col-end-13'"
          class="flex flex-col gap-1">
          <p :class="i % 2 == 0
            ? 'bg-gray-400 dark:bg-gray-800 rounded-tl-none rounded-lg'
