...
<template>
  <div class="max-w-2xl mx-auto p-4 flex flex-col gap-4 mt-12 md:p-0">
    ...
    <div class="flex justify-between w-full items-center p-4 relative">
      <div class="flex  rounded-full p-1 z-10 bg-green-400">
        <UButton icon="i-heroicons-cog-6-tooth" size="sm" color="black" variant="link" :padded="false"
          class="!text-black" @click="progres = 0" />
      </div>
      <div class="flex  rounded-full p-1 z-10" :class="progres >= 1 ? 'bg-green-400' : 'bg-gray-500'">
        <UButton icon="i-heroicons-user" size="sm" color="black" variant="link" :padded="false" @click="progres = 1"
    ...
    <UButton :label="progres != 2 ? 'Next step' : 'Finish'" size="lg" block class="mt-4" @click="incrementProgress" />
  </div>
</template>